...
import com.moselin.rmlib.util.ColorUtils;
...
public class MaterialBackgroundDetector {
    ...
    private void resetPaint() {
        ...
        mCirclePaint.setColor(mFocusColor);
    }
    ...
    private void startShadowAnimation(MotionEvent event) {
        ...
        mAnimator.start();
        if (DBG) {
            L.i("Down,from:" + 0 + ",to:" + mViewRadius);
        }
    }
    ...
    private void cancelShadowAnimation() {
        ...
        if (duration > 0) {
            //When the fingers leave the view, if the mask doesn't cover whole view, we let the mask appear fast.
                ...
                mAnimator.start();
            if (DBG) {
                L.i("UP,from:" + mRadius + ",to:" + mViewRadius);
            }
        }
        if (duration > 0) {
            showAlphaAnimation();
...
//                float rightY = mCenterX+mRadius;
//                if (leftX < 0)
//                    leftX = 0f;
//                rectf.set(leftX,mCenterY-mView.getTop(), rightY,
//                        mView.getTop()+ mView.getTop()/2+mCenterY);
//                L.v(rectf.left+"=="+rectf.top+"=="+rectf.right+"=="+rectf.bottom);
//                L.v(cornerRadius+"=="+cornerRadius);
