...
/**
 * Created by Moselin on 2016/12/7.
 */
...
public class RefreshLoadLayout extends RelativeLayout
{
    ...
    //	// 下拉头
    private RefreshHolder refreshView;
	// 上拉头
    private RefreshHolder loadmoreView;
    ...
    public void loadmoreFinish(int refreshResult)
    {
        ...
        switch (refreshResult)
        {
            case SUCCEED:
                // 加载成功
                loadmoreView.refreshComplete();
                break;
            case FAIL:
            default:
                // 加载失败
                loadmoreView.refreshFail();
                break;
        }
        if (pullUpY < 0)
        {
    ...
    private void changeState(int to)
    {
        ...
        switch (state)
        {
            ...
            case RELEASE_TO_LOAD:
                // 释放加载状态
                loadmoreView.canRefresh();
                break;
            case LOADING:
                // 正在加载状态
                loadmoreView.beginRefresh();
                break;
            case DONE:
                // 刷新或加载完毕，啥都不做
                break;
                ...
                if (mEvents == 0)
                {
                            ...
                            && canPullDown && state != LOADING))
                    {
                        ...
                        if (state == REFRESHING)
                        {
                            ...
                            || (!canChildScrollDown() && canPullUp && state != REFRESHING))
                    {
                        // 可以上拉，正在刷新时不能上拉
            ...
            AsyncTask<Integer, Float, String>
    {
        ...
        protected void onPostExecute(String result)
        {
            ...
            // 刷新操作
            if (mListener != null)
                mListener.onRefresh(RefreshLoadLayout.this);
            hide();
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b)
    {
        if (!isLayout)
        {
            ...
            isLayout = true;
            refreshDist = refreshView.getViewHeight();
            loadmoreDist = loadmoreView.getViewHeight();
        }
    ...
    public interface OnRefreshListener
    {
         ...
         * 刷新操作
         */
        void onRefresh(RefreshLoadLayout pullToRefreshLayout);

        /**
         * 加载操作
         */
        void onLoadMore(RefreshLoadLayout pullToRefreshLayout);
    }
    /**
     * @return 子视图是否可以下拉
     */
